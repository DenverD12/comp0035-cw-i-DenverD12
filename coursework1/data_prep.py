...
def load_data_arrivals():
    ...
    arrivals_csv = \
        Path(__file__).parent.parent.joinpath('coursework1', 'data', 
        'arrivals.csv')
...
def make_list_country_names():
    ...
    country_names_worldbank  = [ "Afghanistan", "Albania", "Algeria",
...
def clean_names_country_names_column(data):
    ...
    df_merged["Country Name"].replace(
        ["Bahamas, The", "Gambia, The"], ["The Bahamas", "The Gambia"], inplace=True)
...
if __name__ == '__main__':
    ...
    df_merged = merge_arrivals_metadata(df_arrivals_dropped_cols, df_income_regions_drop_cols)
    df_columns_reordered = reorder_columns(df_merged)
    df_country_names_worldbank = make_list_country_names()
    
    df_merged_cleaned_1st_column = \
        country_names_clean(df_merged, df_country_names_worldbank)
